...
public function mostrarCitasCliente()
{
    ...
    $comboSeleccionado = request()->query('comboId', ''); // ✅ Cambiar de 'combo' a 'comboId'
    
    // ✅ CARGAR DATOS DEL COMBO SI EXISTE
    $comboData = null;
    if ($comboSeleccionado) {
        $combo = \App\Models\Combo::with('servicios')->find($comboSeleccionado);
        if ($combo) {
            $comboData = [
    ...
    return view('cliente.citasCliente', compact(
        ...
        'promoSeleccionada',
        'comboSeleccionado',
        'comboData' // ✅ Pasar datos del combo a la vista
    ));
}
